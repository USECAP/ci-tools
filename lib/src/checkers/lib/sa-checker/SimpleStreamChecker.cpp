...
class StopTrackingCallback final : public SymbolVisitor {
 ...
 public:
  explicit StopTrackingCallback(ProgramStateRef st) : state(std::move(st)) {}
...
SimpleStreamChecker::SimpleStreamChecker()
  ...
  DoubleCloseBugType = std::make_unique<clang::ento::BugType>(
      this, "Double fclose", "Unix Stream API Error");

  LeakBugType = std::make_unique<clang::ento::BugType>(this, "Resource Leak",
...
void SimpleStreamChecker::checkPostCall(const CallEvent &Call,
  ...
  if (!Call.isCalled(OpenFn)) return;
...
void SimpleStreamChecker::checkPreCall(const CallEvent &Call,
  ...
  if (!Call.isCalled(CloseFn)) return;
...
void SimpleStreamChecker::checkDeadSymbols(SymbolReaper &SymReaper,
  ...
  for (StreamMapTy::iterator I = TrackedStreams.begin(),
    ...
    if (IsSymDead) State = State->remove<StreamMap>(Sym);
  }
...
void SimpleStreamChecker::reportLeaks(ArrayRef<SymbolRef> LeakedStreams,
  ...
  // TODO(sirko): Identify the leaked file descriptor.
...
bool SimpleStreamChecker::guaranteedNotToCloseFile(
  ...
  if (!Call.isInSystemHeader()) return false;

  // Handle cases where we know a buffer's /address/ can escape.
  return !Call.argumentsMayEscape();
